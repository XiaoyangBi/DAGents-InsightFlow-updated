...
- 不要假设某类公开信息不存在。
- 总查询数量建议保持在 4 到 8 条；如果确有必要可以提供更多，运行时会按优先级截断。
- recovery_query_templates 默认提供 2 条高质量备选；如果某个 intent 确实需要更多备选可以继续提供，运行时会按预算裁剪。
"""
...
def _merge_with_required_coverage(
    ...
    planned_specs = _dedupe_specs(planned.queries)
