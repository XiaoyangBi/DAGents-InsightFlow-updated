...
async def test_create_workflow(client: AsyncClient):
    ...
    resp = await client.post(
        "/api/v1/workflows?title=测试竞品分析项目",
...
async def test_list_workflows(client: AsyncClient):
    token = await get_auth_token(client)
    await client.post("/api/v1/workflows?title=项目1", headers={"Authorization": f"Bearer {token}"})
    await client.post("/api/v1/workflows?title=项目2", headers={"Authorization": f"Bearer {token}"})
